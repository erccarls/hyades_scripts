...
def GenGaldef(
    ...
    t_step=50,
    n_steps=1000,
    continuous=0,  # 0 is impulse,  1: continuous source
    ):
...
start_timestep       ='''+str(t_step)+'''
  end_timestep       ='''+str(t_step)+'''
timestep_factor      =0.25         
timestep_repeat      =0   number of repeats per timestep in  timetep_mode=1
timestep_repeat2     ='''+str(n_steps)+'''    number of timesteps in timetep_mode=2
timestep_print       =1000000  number of timesteps between printings
timestep_diagnostics =1000000  number of timesteps between diagnostics
...
source_specification = 1     2D::1:r,z=0 2:z=0  3D::1:x,y,z=0 2:z=0 3:x=0 4:y=0
source_model         = 7     0=zero 1=parameterized 2=case-B 3=pulsars 5=S-Mattox 6=S-Mattox with cutoff 7=Gaussian 8=Table 9=HI+H2 10=H2 11=HII
source_parameters_0  = 0     not used
source_parameters_1  = 0.0     model 1:alpha
source_parameters_2  = .01     model 1:beta
source_parameters_3  = .01     model 1:rmax
source_parameters_4  = 0.01     model 1:rmax
source_parameters_5  = 0.0     model 1:rmax
source_parameters_6  = 0     not used
source_parameters_7  = 0     not used
source_parameters_8  = 0     not used
source_parameters_9  = '''+str(continuous)+'''     Added by Eric Carlson: 0:impulse source on first timestep. !=0: continuous source.

source_model_elec    = 1     source model for electrons, definitions as for nuclei
source_pars_elec_0   = 0     not used
source_pars_elec_1   = 0.0     model 1:alpha
source_pars_elec_2   = .01     model 1:beta
source_pars_elec_3   = .01     model 1:rmax
source_pars_elec_4   = .01     model 1:rmax
source_pars_elec_5   = 0.0     model 1:rmax
source_pars_elec_6   = 0     not used
source_pars_elec_7   = 0     not used
source_pars_elec_8   = 0     not used
source_pars_elec_9   = 0     not used
...
for HIModel,H2Model in [(2,2),(1,1),]:
    ...
    for total_t in np.logspace(3,8,11):
        ...
        t_step = int(total_t/float(n_steps))

        GenGaldef(
            filename=prefix+str(count),   # filename for output files and for galdef suffix
            HIModel=HIModel,
            H2Model=H2Model,  
            dx=.05, # kpc for dx and dy propagation grid
            dz=.05, # kpc for dz propagation grid
            zmax=3.5, # halo half-height 
            continuous = 0, 
            t_step=t_step, 
            n_steps=n_steps,
