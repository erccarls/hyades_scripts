...
cat <<EOS | qsub -V -q hyper -S /bin/bash -N galprop_"""+filename+""" -l nodes=1:ppn=32,walltime=72:00:00 - 
...
for HIModel,H2Model in [(1,1),(2,2)]:
    ...
    for spiral_fraction in np.linspace(0,.5,6):
        GenGaldef(
            filename=prefix+str(count),   # filename for output files and for galdef suffix
            HIModel=HIModel,
            H2Model=H2Model,  
